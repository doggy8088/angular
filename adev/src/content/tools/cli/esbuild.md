...
Vite 在 Angular CLI 中的使用目前僅限於 _開發伺服器容量_。即使不使用底層的 Vite 建構系統，Vite 也提供了一個功能齊全的開發伺服器，並具有已捆綁到低依賴 npm 套件中的客戶端側支援。這使其成為提供全面的開發伺服器功能的理想候選者。當前的開發伺服器程序使用新的建構系統在內存中自動建立應用程式的開發建構，並將結果傳遞給 Vite 以服務應用程式。Vite 的使用與基於 Webpack 的開發伺服器非常相似，被封裝在 Angular CLI `dev-server` 建構器中，目前無法直接配置。
