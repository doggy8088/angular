...
水合作用會對您的應用程式施加一些限制，而在未啟用水合作用時則不會出現這些限制。您的應用程式在伺服器和用戶端都必須具有相同的生成 DOM 結構。水合作用的程序預期在兩個地方 DOM 樹具有相同的結構。這也包含 Angular 在伺服器上進行渲染時產生的空白和註解節點。這些空白和節點必須存在於伺服器端渲染程序所產生的 HTML 中。
