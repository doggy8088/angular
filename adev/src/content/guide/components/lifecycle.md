...
組件的 **生命週期** 是發生在組件建立與銷毀之間的一系列步驟。每個步驟都代表了 Angular 渲染組件和隨時間檢查它們以進行更新的不同部分的過程。
...
`ngOnInit` 方法在 Angular 以其初始值初始化所有組件輸入之後執行。組件的 `ngOnInit` 僅執行一次。
...
您可以將 `DestroyRef` 實例傳遞給組件外部的函數或類別。如果您有其他程式碼應該在組件被銷毀時執行一些清理行為時，請使用此模式。
...
`ngAfterContentInit` 方法在組件內嵌套的所有子代 (其 _內容_) 初始化後執行一次。
